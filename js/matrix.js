...
function Matrix(container, parameters){
    ...
    var range_x= params.range_x || [-4, 4];
    var range_y= params.range_y || [-1, 1];
    ...
    var dx = (range_x[1] - range_x[0])/qds[0];
    var dy = (range_y[1] - range_y[0])/qds[1];
    ...
    for(var i=0; i<=qds[0]; i++) {
        svg.append("svg:text")
            ...
            .attr("text-anchor", "middle")
            .text(formatFloat(range_x[0] + dx * i));
    }
    ...
    for(var j=0; j<=qds[1]; j++) {
        svg.append("svg:text")
            ...
            .attr("text-anchor", "middle")
            .attr("transform", "rotate(-90)")
            .text(formatFloat(range_y[1] - (dy * j)));
    }

    var rx = (x - range_x[0])/(range_x[1] - range_x[0]);
    var ry = (y - range_y[0])/(range_y[1] - range_y[0]);
